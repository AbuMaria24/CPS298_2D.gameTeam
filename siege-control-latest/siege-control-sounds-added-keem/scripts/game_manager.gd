...
# Wave list
# Wave 1: slimes only
# Wave 2: slimes + goblins
# Wave 3: slimes + goblins
# Wave 4: slimes + goblins + flying eyes
...
func _start_wave() -> void:
	...
	# First spawn interval = whole wave duration / number of enemies
...
func _on_spawn_tick() -> void:
	...
	# Schedule next spawn based on remaining time and enemies
...
func _spawn_one() -> void:
	...
	var is_flying: bool = (enemy_scene == FLYING_EYE_SCENE)
	...
	if config.load("user://settings.cfg") == OK:
		for action in ["move_left", "move_right", "jump", "attack"]:
			...
			if key_name != "":
				...
				# Convert "A", "SPACE", "LEFT", etc. to a keycode
				var keycode = DisplayServer.keyboard_get_keycode_from_physical(key_name)
